    ...
    stages {
        stage('Build') {
            steps {
                ...
                retry(3){
                    // Packer Build
                    sh './packer build -machine-readable -var aws_access_key=$AWS_ACCESS_KEY_PSW -var aws_secret_key=$AWS_SECRET_KEY_PSW TerraformAndPackerSample/aws-template.json | tee build.log'
                }
            }
        }
    }
